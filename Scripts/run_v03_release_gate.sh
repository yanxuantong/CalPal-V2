...
TEST_DERIVED_DATA_PATH="${TEST_DERIVED_DATA_PATH:-/tmp/CalPalV03ReleaseGateTests}"
...
if ! grep -q "does not collect data from this app" "$PRIVACY_POLICY_PATH"; then
  echo "AppStore/PRIVACY_POLICY.md must state the current v0.3 data collection posture." >&2
  exit 1
fi

if ! grep -q "does not track users" "$PRIVACY_POLICY_PATH"; then
  echo "AppStore/PRIVACY_POLICY.md must state the current v0.3 tracking posture." >&2
  exit 1
fi
...
for screenshot in calpal-demo-home.png calpal-demo-home-dark.png; do
  ...
  if [[ ! -s "$path" ]]; then
    ...
    echo "Run CAPTURE_SCREENSHOTS=1 bash Scripts/run_v03_release_gate.sh or bash Scripts/capture_demo_screenshots.sh." >&2
    exit 1
  fi
