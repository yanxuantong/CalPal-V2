...
- `Scripts/create_release_evidence_artifacts.sh` creates dated repo-local evidence skeletons for signed upload, TestFlight device smoke, screenshot review, metadata, and privacy answers; the signed-upload skeleton includes the canonical `Artifacts/AppStoreUpload/CalPal-<version>-<build>.xcarchive` path and `xcodebuild -exportArchive` method expected by the public-release verifier.
