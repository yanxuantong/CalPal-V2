...
final class MVPBugFixRegressionTests: XCTestCase {
    ...
    func testSceneActivationResumesInterruptedResultDismissal() async throws {
        let repo = MockCalendarRepository()
        let event = CalendarEvent(
            id: "resumed-visible-result",
            title: "Saved before resume",
            calendarID: "work",
            calendarName: "Work Calendar",
            accountName: "iCloud",
            startDate: PreviewFixtures.now,
        ...
        let pipeline = DelayedCommandPipeline(
            output: .result(CommandResultViewState(title: "Added to Calendar", message: "Resumed result", event: event, actionTitle: "Open in Calendar")),
            delayNanoseconds: 0
        )
        let speech = MockSpeechService()
        let dependencies = DependencyContainer(
        ...
        await model.submit(text: "Save a resumed result")
        model.cancelActiveWorkForSceneInterruption()
        try await Task.sleep(nanoseconds: 120_000_000)
        XCTAssertEqual(model.latestResult?.message, "Resumed result")

        model.resumeResultDismissalAfterSceneActivation()
        try await Task.sleep(nanoseconds: 120_000_000)
...
final class V2UsabilityRegressionTests: XCTestCase {
    ...
    func testCancelRecordingSuppressesLateSpeechAuthorizationDenial() async throws {
        let repo = MockCalendarRepository()
        let speech = MockSpeechService(
            authorization: .denied,
            authorizationDelayNanoseconds: 160_000_000
        )
        let pipeline = CountingCommandPipeline(output: .failure(ErrorPresentation(title: "Unexpected", message: "Should not parse", recovery: nil)))
        let deps = DependencyContainer(
            calendarRepository: repo,
    ...
    func testCancelRecordingStopsLateSpeechStartCompletion() async throws {
        let repo = MockCalendarRepository()
        let speech = MockSpeechService(startDelayNanoseconds: 160_000_000)
        let pipeline = CountingCommandPipeline(output: .failure(ErrorPresentation(title: "Unexpected", message: "Should not parse", recovery: nil)))
        let deps = DependencyContainer(
            calendarRepository: repo,
            commandPipeline: pipeline,
        ...
        XCTAssertEqual(speech.requestAuthorizationCount, 1)
        XCTAssertEqual(speech.startTranscriptionCount, 1)
        XCTAssertEqual(speech.cancelTranscriptionCount, 2)
    }
