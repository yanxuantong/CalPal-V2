    ...
    func process(text: String) async -> CalendarCommandPipelineOutput {
        ...
        if parsed.requiresCalendarRead {
            ...
            guard status == .allowed else {
                return .unavailable(UnavailableContext(title: "Calendar Access Needed", message: "Full access is required to find, modify, or delete existing events.", primaryAction: .openSystemSettings, secondaryAction: .openSettings))
            }
        }
