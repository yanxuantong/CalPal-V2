...
final class CalendarMutationPolicy {
    ...
    func decide(parsed: ParsedCalendarCommand, calendars: [CalendarInfo], repository: CalendarRepositoryProtocol) async -> CommandDecision {
        ...
        if parsed.confidence < 0.7 {
            if case .create(let draft) = intent {
                return .needsCorrection(CorrectionContext(title: "Review Event Details", message: "Some required details were missing or uncertain.", draft: applyDefaultCalendar(draft, calendars: calendars), missingFields: parsed.missingFields + parsed.warnings, sourceText: parsed.originalText))
            }
            return .needsCorrection(defaultCorrection(parsed: parsed, calendars: calendars))
        }
        switch intent {
        case .create(let draft):
            ...
            guard prepared.hasRequiredFields else {
                return .needsCorrection(CorrectionContext(title: "Complete Event", message: "Add the missing fields before saving.", draft: prepared, missingFields: parsed.missingFields, sourceText: parsed.originalText))
            }
            if !parsed.warnings.isEmpty {
                return .needsCorrection(CorrectionContext(title: "Check Date", message: parsed.warnings.joined(separator: "\n"), draft: prepared, missingFields: parsed.warnings, sourceText: parsed.originalText))
            }
            return .autoApply(prepared)
        case .modify(let query, let patch):
            return await candidateDecision(operation: .modify, query: query, patch: patch, sourceText: parsed.originalText, repository: repository)
        case .delete(let query):
            return await candidateDecision(operation: .delete, query: query, patch: nil, sourceText: parsed.originalText, repository: repository)
        }
    }

    private func candidateDecision(operation: CommandOperation, query: EventQuery, patch: EventPatch?, sourceText: String, repository: CalendarRepositoryProtocol) async -> CommandDecision {
        do {
            let candidates = try await repository.searchEvents(query: query)
            if candidates.count == 1, let target = candidates.first {
                return .needsConfirmation(confirmationContext(operation: operation, target: target, patch: patch))
            }
            if candidates.isEmpty {
                return .needsCorrection(CorrectionContext(title: "No Matching Event", message: "Refine the event title or choose a manual fallback.", draft: EventDraft(title: query.titleHint ?? "", startDate: query.day ?? now(), endDate: (query.day ?? now()).addingTimeInterval(3600), calendarID: nil, calendarName: nil, location: nil, notes: nil), missingFields: ["matching event"], sourceText: sourceText))
            }
            return .needsCandidateSelection(CandidateSelectionContext(operation: operation, candidates: candidates, patch: patch, sourceText: sourceText))
        } catch {
    ...
    private func confirmationContext(operation: CommandOperation, target: CalendarEvent, patch: EventPatch?) -> ConfirmationContext {
